...
export default function MobileSidebar({ open, onClose }) {
  ...
  const [openGroup, setOpenGroup] = useState({ myData: true, guild: true });

  return (
    <Dialog open={open} onClose={onClose} className="relative z-50">
      <div className="fixed inset-0 bg-black/30" aria-hidden="true" />
      <div className="fixed inset-0 flex items-center justify-start">
        <Dialog.Panel className="w-64 bg-gray-900 border-r border-gray-800 shadow-xl h-full overflow-y-auto">
              ...
              <nav className="space-y-2">
                <div>
                  <button
                    ...
                    onClick={() => setOpenGroup(o => ({ ...o, myData: !o.myData }))}
                  >
                    {MyDataIcon}
                    <span className="font-semibold text-white flex-1 text-left">My Data</span>
                    <span className={`transition-transform ${openGroup.myData ? 'rotate-90' : ''}`}>▶</span>
                  </button>
                  <div className={`overflow-hidden transition-all duration-300 ${openGroup.myData ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}
                ...
                <div>
                  <button
                    className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors bg-gray-800/60 hover:bg-blue-900/40 mb-1`}
                    onClick={() => setOpenGroup(o => ({ ...o, guild: !o.guild }))}
                  >
                    {GuildIcon}
                    <span className="font-semibold text-white flex-1 text-left">Guild</span>
                    <span className={`transition-transform ${openGroup.guild ? 'rotate-90' : ''}`}>▶</span>
                  </button>
                  <div className={`overflow-hidden transition-all duration-300 ${openGroup.guild ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}
                       style={{willChange: 'max-height'}}>
                    <div className="flex flex-col gap-1 overflow-y-auto">
                      {guildItems.map(item => (
                        <button
